...
export const locales = {
  zh: {
    ...
    sessionDescription:
      '跟随当前会话最近一次请求所使用的模型；失败达到设定次数后回退到下面配置的模型。',
    configuredDescription: '使用下面单独配置的 provider 和 model。',
    fallbackAttempts: '当前会话失败次数后回退到自定义模型',
    provider: 'Provider',
    model: 'Model',
    fallbackProvider: '回退 Provider',
    fallbackModel: '回退 Model',
    fallbackMissing: '未填写回退模型时，会话模型失败会直接报错。',
  ...
  en: {
    ...
    sessionDescription:
      "Follow the model used by the current session's latest request; after the configured number of failures, fall back to the model below.",
    ...
    fallbackMissing:
      'Without a fallback model, a session-model failure surfaces directly.',
